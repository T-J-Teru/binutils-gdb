				 ...
				 const char *value)
{
  ...
  if (use_compressed_breakpoints == AUTO_BOOLEAN_AUTO)
    if (riscv_isa_has_feature (gdbarch, 'C'))
...
riscv_isa_has_feature (struct gdbarch *gdbarch, char feature)
{
  ...
  /* It would be nice to always check with the real target where possible,
     ...
     GDBARCH and we have to decide at that point if we should decrement by
     2 or 4 bytes.  As the target is not running at that point, we use the
     abi information from the bfd object to pick a suitable answer.

     If the bfd was not compiled with compressed instruction support, but
     the target does support compressed instructions, then we will have set
     decr_pc_after_break to 4 bytes, but we will then plant 2-byte
     compressed breakpoints.  The result is that when we hit a breakpoint
     we will decrement $pc by 4-bytes and end up in the wrong place.

     It is safer then to always make compressed instruction support
     decisions based on the abi rather than the actual hardware.  */
  if (feature != 'C')
  ...
  /* If we could not read the misa register from the target, or the value 0
     ...
     Hopefully, if the user knows what they are doing then the abi
     features should be a subset of the actual features, so for all
     current uses this is probably fine.  */
