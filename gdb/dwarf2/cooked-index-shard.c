...
cooked_index_shard::resolve_deferred_parents (const parent_map_map *parent_maps)
{
  for (cooked_index_entry *entry : m_entries)
      ...
      if ((entry->flags & IS_PARENT_DEFERRED) != 0)
	{
      ...
      /* Remove a parent reference if the parent has no name.  This leaves
	 ENTRY as an orphan, but this only happens if the DWARF is corrupted
	 and we failed to find a name for the parent.  We can safely check
	 the parent's name at this point because all deferred names will
	 have been resolved in all shards before this function is called on
	 any shard.  */
  ...
  for (const auto &[entry, signature] : m_deferred_names)
    {
	 ...
	 and the entry should only be added when it has no name.  */
      ...
      /* Find and set the name for ENTRY using its SIGNATURE.  */
      if (const auto it = sig_names.find (signature);
	  it != sig_names.end ())
	entry->name = it->second;
      else
	{
	  /* A name for ENTRY could not be found.  */
