...
private:
  ...
  /* Called after finalize has completed, store any complaints that
     were generated.  */
  void merge_finalize_complaints (complaint_collection &&complaints)
  {
    if (m_finalize_complaints.empty ())
      ...
      {
	/* The current version of gdb::unordered_set doesn't suppot
  ...
  /* Return the set of complaints emitted during the call to finalize.
     We move these complaints out of the shard as these are only
     emitted once, and don't need to be stored beyond that.  */
