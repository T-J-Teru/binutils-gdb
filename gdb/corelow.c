...
public:
  ...
  void validate_files () const
  {
    ...
    if (exec != nullptr)
      {
	if (!core_file_matches_executable_p (cbfd, exec))
	  warning (_("core file may not match specified executable file."));
	else if (gdb_bfd_get_mtime (exec) > gdb_bfd_get_mtime (cbfd))
...
bfd *
get_inferior_core_file (inferior *inf)
{
...
core_file_command (const char *filename, int from_tty)
{
  ...
  if (filename == NULL)
    {
      if (get_current_core_target () != nullptr)
	{
...
core_target_open (const char *arg, int from_tty)
{
  ...
  if (current_program_space->exec_bfd () == nullptr)
    locate_exec_from_corefile_build_id (target->core_bfd (),
					target, ctx, from_tty);
					...
					bool required)
{
  ...
  section = bfd_get_section_by_name (this->core_bfd (),
				     section_name.c_str ());
  if (! section)
    {
      if (required)
  ...
  if (!bfd_get_section_contents (this->core_bfd (), section,
			   ...
			   ULONGEST offset, ULONGEST len, ULONGEST *xfered_len)
{
  switch (object)
      ...
      if (readbuf)
	{
	  ...
	  if (size == 0)
	    return TARGET_XFER_EOF;
	  if (!bfd_get_section_contents (this->core_bfd (),
  ...
  if (m_core_gdbarch && gdbarch_core_read_description_p (m_core_gdbarch))
    {
      ...
      result = gdbarch_core_read_description
		 (m_core_gdbarch, this, m_core_bfd.get ());
      if (result != nullptr)
	return result;
    }
...
core_target::info_proc (const char *args, enum info_proc_what request)
{
  ...
  if (gdbarch_core_info_proc_p (gdbarch))
    gdbarch_core_info_proc (gdbarch, this->core_bfd (),
			    args, request);
...
core_target::supports_memory_tagging ()
{
  ...
  return (bfd_get_section_by_name (this->core_bfd (), "memtag")
	  != nullptr);
}
			    ...
			    gdb::byte_vector &tags, int type)
{
  ...
  while (get_next_core_memtag_section (this->core_bfd (),
