...
/* Implement the gdbarch core_parse_exec_context method.  Try to extract
   the inferior arguments, environment, and executable name from CBFD.  */

static core_file_exec_context
linux_core_parse_exec_context_1 (struct gdbarch *gdbarch, bfd *cbfd)
{
  ...
  /* Parse the .auxv section looking for the AT_EXECFN attribute.  The
     ...
     executable name.  Additionally, this string is placed at the top of
     the program stack, and so will be in the same PT_LOAD segment as the
     argv and envp arrays.  We can use this to try and locate these
     arrays.  If we can't find the AT_EXECFN attribute then we're not
     going to be able to do anything else here.  */
  CORE_ADDR exec_name_string_addr;
  if (target_auxv_search (contents, current_inferior ()->top_target (),
			  gdbarch, AT_EXECFN, &exec_name_string_addr) != 1)
    return {};
  ...
  /* Now scan through the headers looking for the one which contains the
     address held in EXEC_NAME_STRING_ADDR, this is the address of the
     executable name pointed too by the AT_EXECFN auxv entry.  */
  Elf_Internal_Phdr *hdr = nullptr;
  for (int i = 0; i < num_phdrs; i++)
    {
      /* The program header that contains the address EXEC_NAME_STRING_ADDR
	 should be one where all content is contained within CBFD, hence
	 the check that the file size matches the memory size.  */
      if (phdrs.get ()[i].p_type == PT_LOAD
	  && phdrs.get ()[i].p_vaddr <= exec_name_string_addr
	  && (phdrs.get ()[i].p_vaddr
	      + phdrs.get ()[i].p_memsz) > exec_name_string_addr
  ...
  /* As we assume the stack grows down (see early check in this function)
     ...
     EXEC_NAME_STRING_ADDR and the segments virtual address.  These define
     the HIGH and LOW addresses between which we are going to search.  */
  CORE_ADDR low = hdr->p_vaddr;
  CORE_ADDR high = exec_name_string_addr;
  ...
  /* Now search down through memory looking for a PTR_BYTES sized object
     which contains the value EXEC_NAME_STRING_ADDR.  The hope is that this
  ...
  while (ptr > low)
    {
      if (deref (ptr) == exec_name_string_addr
	  && (ptr - ptr_bytes) > low
  ...
  /* Skip the first argument.  This is the executable name, but we'll load
  ...
  gdb::unique_xmalloc_ptr<char> exec_name
    = target_read_string (exec_name_string_addr, INT_MAX);
...
/* Wrapper around linux_core_parse_exec_context_1 which catches and
   discards memory errors, if we're probing to find the environment,
   arguments, and executable name from a core-file, and we fail to read
   some memory then we should just carry on without that information.  */

static core_file_exec_context
linux_core_parse_exec_context (struct gdbarch *gdbarch, bfd *cbfd)
{
  try
    {
      return linux_core_parse_exec_context_1 (gdbarch, cbfd);
    }
