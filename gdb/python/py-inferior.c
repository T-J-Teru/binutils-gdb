...
static PyObject *
infpy_build_ids (PyObject *obj, PyObject *args, PyObject *kw)
{
  inferior_object *self = (inferior_object *) obj;

  INFPY_REQUIRE_VALID (self);

  PyObject *all_files_obj = Py_False;
  static const char *keywords[] = { "all_files", nullptr };
  if (!gdb_PyArg_ParseTupleAndKeywords (args, kw, "|O!", keywords,
					&PyBool_Type, &all_files_obj))
    return nullptr;
  bool all_files = all_files_obj == Py_True;
  ...
  for (const auto &it : build_id_and_filename_list)
    {
      if (!all_files && it.build_id () == nullptr)
	continue;
	...
	{
	  Py_INCREF (Py_None);
	  if (PyTuple_SetItem (tuple.get (), 1, Py_None) != 0)
	    return nullptr;
	}
...
Unset an environment of this inferior." },
  { "build_ids", (PyCFunction) infpy_build_ids, METH_VARARGS | METH_KEYWORDS,
    "build_ids () -> List\n\
