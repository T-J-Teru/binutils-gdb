...
show_libthread_db_search_path (struct ui_file *file, int from_tty,
			       ...
			       const char *value)
{
  gdb_printf (file, _("The current search path for libthread_db is \"%s\".\n"),
	      value);
}
...
{
  gdb_printf (file,
	      _("Whether to check libthread_db at load time is \"%s\".\n"),
	      value);
}
...
Show the current search path for libthread_db."), _("\
