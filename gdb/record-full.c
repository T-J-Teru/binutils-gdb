...
/* Open the process record target for 'core' files.  CBFD is the core file
   containing the record information, which must not be NULL.  */
...
/* Restore the execution log from core file CBFD, which must not be
   NULL.  */
      ...
      while (1)
	{
	  ...
	  switch (rectype)
	    ...
	    case record_full_end: /* end */
	      ...
	      /* Get signal value.  */
	      bfdcore_read (cbfd, osec, &signal, sizeof (signal), &bfd_offset);
