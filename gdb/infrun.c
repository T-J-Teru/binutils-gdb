...
prepare_to_proceed (int step)
{
  ...
  gdb_assert (! non_stop || inferior_stop);
...
get_last_target_status (ptid_t *ptidp, struct target_waitstatus *status)
{
  struct target_status *info;
  if (have_inferiors()
      && (info = inferior_data (current_inferior (), infrun_inferior_data)))
    {
      if (ptidp)
...
handle_inferior_event (struct execution_control_state *ecs)
{
  ...
  if (!non_stop)
    ...
    {
      if (inferior_stop)
	set_executing (pid_to_ptid (ptid_get_pid (ecs->ptid)), 0);
      else
  ...
  find_pc_partial_function (stop_pc, &ecs->stop_func_name, //added by kdavis@cray.com
   ...
   ecs->stop_func_start
    += gdbarch_deprecated_function_start_offset (gdbarch);//added by kdavis@cray.com
...
process_event_stop_test:
  ...
  if (stop_pc >= ecs->event_thread->control.step_range_start /**************************************************************************************/
      && stop_pc < ecs->event_thread->control.step_range_end && (execution_direction != EXEC_REVERSE
	  || frame_id_eq (get_frame_id (frame), ecs->event_thread->control.step_frame_id)))
    {
      if (debug_infrun)
	fprintf_unfiltered
  ...
  if (!breakpoints_always_inserted_mode () && target_has_execution)
    {
      int remove_status;
      if (inferior_stop)
        remove_status = remove_breakpoints_pid (ptid_get_pid (inferior_ptid));
      else
        remove_status = remove_breakpoints ();
      if (remove_status)
	{
