...
struct i386_linux_nat_target final : public x86_linux_nat_target
{
  ...
  enum target_xfer_status xfer_partial (enum target_object object,
					...
					ULONGEST offset, ULONGEST len, ULONGEST *xfered_len) override;
};
				    ...
				    ULONGEST offset, ULONGEST len, ULONGEST *xfered_len)
{
  if (object == TARGET_OBJECT_X86_LINUX_TLS_DESC)
    return x86_linux_nat_target::xfer_tls_desc (readbuf, writebuf, offset, len, xfered_len, 6);

  return x86_linux_nat_target::xfer_partial (object, annex, readbuf, writebuf, offset, len, xfered_len);
}
