...
/* Get the current terminal data.  If none is found yet, add it now.  This
...
new_tty_postfork (void)
{
  ...
  if (!inferior_thisrun_terminal.empty ())
    {
      run_terminal->ttyname = make_unique_xstrdup (inferior_thisrun_terminal.c_str ());
...
set_sigint_trap (void)
{
  ...
  if (inf->attach_flag || tinfo->run_terminal != nullptr)
    {
