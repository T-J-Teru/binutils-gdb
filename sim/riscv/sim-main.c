  ...
  if ((op->match & mask_ebk_jalr_add) == match_ebk_jalr_add)
      ...
      if (iw == MATCH_C_EBREAK)
	{
	  ...
	  TRACE_INSN (cpu, "c.break");
          /* GDB expects us NOT to haved stepped over the C_EBREAK.  */
...
execute_i (SIM_CPU *cpu, unsigned_word iw, const struct riscv_opcode *op)
{
  ...
  switch (op->match)
    ...
    case MATCH_CSRRC:
      ...
      switch (csr)
	{
#define DECLARE_CSR(name, num, _class, _from_spec, _to_spec)     \
	case num: \
...
#undef DECLARE_CSR
    ...
    case MATCH_SBREAK:
      ...
      /* GDB expects us NOT to haved stepped over the SBREAK.  */
      sim_engine_halt (sd, cpu, NULL, cpu->pc, sim_stopped, SIM_SIGTRAP);
      break;
    case MATCH_ECALL:
...
execute_one (SIM_CPU *cpu, unsigned_word iw, const struct riscv_opcode *op)
{
  ...
  switch (op->insn_class)
    ...
    case INSN_CLASS_NONE:
    case INSN_CLASS_Q:
    default:
      TRACE_INSN (cpu, "UNHANDLED EXTENSION");
      sim_engine_halt (sd, cpu, NULL, cpu->pc, sim_signalled, SIM_SIGILL);
    }
...
void initialize_cpu (SIM_DESC sd, SIM_CPU *cpu, int mhartid)
{
  ...
  /* Set the width field within the misa register.  */
  if (RISCV_XLEN (cpu) == 32)
    cpu->csr.misa |= (unsigned_word) 1 << ((sizeof (cpu->csr.misa) * 8) - 2);
  else if (RISCV_XLEN (cpu) == 64)
    cpu->csr.misa |= (unsigned_word) 2 << ((sizeof (cpu->csr.misa) * 8) - 2);
  else if (RISCV_XLEN (cpu) == 128)
    cpu->csr.misa |= (unsigned_word) 3 << ((sizeof (cpu->csr.misa) * 8) - 2);
