...
/* A template class that implements 'scope_exit' like things.  The POLICY
   ...
   When scope_exit_base is destructed, on_exit is called so long as release
   has not firs been called.  The on_exit function needs to be implemented
   by the POLICY class.
...
public:
  ...
  void operator= (scope_exit_base &&) = delete;
  scope_exit_base (scope_exit_base &&) = delete;
...
public:
  ...
  template<typename EFP,
	   typename = gdb::Requires<std::is_constructible<EF, EFP>>>
  scope_exit_policy (scope_exit_policy &&rhs)
  ...
  void operator= (scope_exit_policy &&) = delete;
  scope_exit_policy (scope_exit_policy &&) = delete;
